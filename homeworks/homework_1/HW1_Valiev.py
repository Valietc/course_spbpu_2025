...
Q = 650  # дебит флюида, м3/cут
q_m3_sec = Q / 86400  # дебит флюида, м3/c
gamma_water = 1.0401989588824798  # относительная плотность по пресной воде с плотностью 1000 кг/м3, безразм
H = 2990.925152101205  # измеренная глубина забоя скважины
d_tub = 0.0909295061141413  # диаметр НКТ, м
angle = 68.00686706066769  # угол наклона скважины к горизонтали, градусы
roughness = 0.0003621459392075182  # шероховатость трубы, м
p_wh = 114.47600387030182 * 101325  # давление на устье, Па
t_wh = 20.59455011706832  # температура на устье скважины, С
temp_grad = 2.2407088842325633  # геотермический градиент, К/м * (1e-2)
h0 = 0

results = calc_pipe(p_wh, t_wh + 273, h0=0, md_vdp=H, temp_grad=temp_grad, gamma_wat=gamma_water, angle=angle,
                    q_ms=q_m3_sec, d_tub=d_tub, roughness=roughness)
results
...
ax.invert_yaxis()
plt.legend()
plt.title("Распределение давления и температуры по длине")
plt.show()
...
plt.ylabel("p_wf, атм")
ax = plt.gca()
plt.title("Зависимость забойного давления от дебита (VLP)")
plt.show()